...
SELECT @input = REPLACE(@input, CHAR(13) + CHAR(10) + CHAR(13) + CHAR(10), '|')
SELECT @input = REPLACE(@input, CHAR(13) + CHAR(10), ' ')
SELECT @input = REPLACE(@input, '  ', ' ')
SELECT @input = REPLACE(@input, ' ', '~')


DECLARE @passport TABLE (passportid INT IDENTITY(1,1), passport_text VARCHAR(MAX), validFieldCount TINYINT DEFAULT 0, fieldTemp VARCHAR(100));


INSERT INTO @passport(passport_text)
SELECT value + '~'
FROM STRING_SPLIT(@input, '|')

DECLARE @PassportSplit TABLE (passportid INT, keyValuePair VARCHAR(100), keyType CHAR(3), keyvalue VARCHAR(100))

INSERT INTO @PassportSplit
(
    passportid,
    keyValuePair
)
...
passportid, value
FROM @passport
CROSS APPLY STRING_SPLIT(passport_text, '~')
WHERE LEN(value) > 0 
...
UPDATE ps
SET
 ps.keyType = SUBSTRING(ps.keyValuePair, 1, CHARINDEX(':',ps.keyValuePair)-1)
,ps.keyvalue = SUBSTRING(ps.keyValuePair, CHARINDEX(':', ps.keyValuePair) +1, 100)
FROM @PassportSplit ps



DELETE FROM @PassportSplit WHERE keyType = 'cid'; --removing all cid records
...
FROM @PassportSplit ps
WHERE keyType = 'byr'
AND ISNULL(TRY_CAST(keyvalue AS INT), 0) NOT BETWEEN 1920 AND 2002


DELETE ps 
FROM @PassportSplit ps
WHERE keyType = 'iyr'
AND ISNULL(TRY_CAST(keyvalue AS INT), 0) NOT BETWEEN 2010 AND 2020

DELETE ps 
FROM @PassportSplit ps
WHERE keyType = 'eyr'
AND ISNULL(TRY_CAST(keyvalue AS INT), 0) NOT BETWEEN 2020 AND 2030
...
(
	CASE 
		WHEN (RIGHT(ps.keyvalue, 2) = 'cm' AND TRY_CAST(REPLACE(ps.keyvalue, 'cm', '') AS INT) BETWEEN 150 AND 193) THEN 1
		WHEN (RIGHT(ps.keyvalue, 2) = 'in' AND TRY_CAST(REPLACE(ps.keyvalue, 'in', '') AS INT) BETWEEN 59 AND 76) THEN 1
		ELSE 0
	END = 0
...
FROM @PassportSplit ps
WHERE keyType = 'hcl'
AND PATINDEX('#[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]', ps.keyvalue) = 0
AND LEN(ps.keyvalue) <> 7

DELETE ps 
FROM @PassportSplit ps
WHERE keyType = 'ecl'
AND ps.keyvalue NOT IN ('amb','blu','brn','gry','grn','hzl','oth')

DELETE ps 
FROM @PassportSplit ps
WHERE keyType = 'pid'
AND PATINDEX('[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]', ps.keyvalue) = 0

SELECT * FROM @PassportSplit ORDER BY keyType, keyvalue
...
SELECT COUNT(*) FROM validPassports