...
DECLARE @Passwords TABLE (String VARCHAR(100), Least INT, Most INT, Letter CHAR(1), PasswordText VARCHAR(100))
...
UPDATE p
SET 
	 p.Least = SUBSTRING(String, 1,CHARINDEX('-',String) -1)
	,p.Most = SUBSTRING(String, CHARINDEX('-', String) +1, CHARINDEX(' ', String) - CHARINDEX('-', String))
...
SELECT 
	'Answer' = COUNT(*)
FROM @Passwords
WHERE LEN(PasswordText) - LEN(REPLACE(PasswordText, Letter, '')) BETWEEN Least AND Most