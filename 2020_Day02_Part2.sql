...
DECLARE @Passwords TABLE (String VARCHAR(100), FirstPos INT, SecondPos INT, Letter CHAR(1), PasswordText VARCHAR(100))

INSERT INTO @Passwords(String)
VALUES
...
UPDATE p
SET 
	 p.FirstPos = SUBSTRING(String, 1,CHARINDEX('-',String) -1)
	,p.SecondPos = SUBSTRING(String, CHARINDEX('-', String) +1, CHARINDEX(' ', String) - CHARINDEX('-', String))
	,p.PasswordText = SUBSTRING(String, CHARINDEX(':',String) + 2, LEN(String))
	,p.Letter = SUBSTRING(String, CHARINDEX(':', String)-1, 1)
FROM @Passwords p
...
FROM @Passwords
WHERE 
	CASE WHEN SUBSTRING(PasswordText, FirstPos, 1) = Letter THEN 1 ELSE 0 END
    + CASE WHEN SUBSTRING(PasswordText, SecondPos, 1) = Letter THEN 1 ELSE 0 END
	= 1