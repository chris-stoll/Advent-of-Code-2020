...
DECLARE @fields TABLE (field VARCHAR(100))
...
DECLARE @passport TABLE (passportid INT IDENTITY(1,1), passport_text VARCHAR(MAX))
...
;WITH fieldCounts AS
(
SELECT p.passportid, f.field
FROM @passport p
CROSS JOIN @fields f
WHERE p.passport_text LIKE '%' + f.field + '%'
),
validPassports AS
(
SELECT fc.passportid
FROM fieldCounts fc
GROUP BY fc.passportid
HAVING COUNT(*) = @fieldCount
)
SELECT 'Answer' = COUNT(*) FROM validPassports